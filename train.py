...
@jax.jit
def bn_vae_train_step(runner_state: Tuple[TrainState, KeyArray], minibatch: jax.Array) -> Tuple[TrainState, KeyArray]:
  ...
  train_metrics = {"loss": loss, "bce_loss": aux_vals[0], "kld_loss": aux_vals[1],
                   "grad_norm": grad_norm}
  state = state.apply_gradients(grads=grads, batch_stats=aux_vals[-1]["batch_stats"])
...
@jax.jit
def bn_vae_eval(config: VAEConfig, rng: KeyArray, state: TrainState, val_inp: jax.Array, latent_vec: jax.Array) -> jax.Array:
  def eval_model(vae_model: VAE):
    img_size = val_inp.shape[-3:]
    recon, mean, logvar = vae_model(rng, val_inp, False)
    comparison = jnp.concatenate([val_inp[:8].reshape(-1, *img_size),
                                  recon[:8].reshape(-1, *img_size)])
    gen_images = vae_model.decode(latent_vec)
    metrics = vae_metrics(recon, val_inp, mean, logvar)

    return metrics, comparison, gen_images
  
  return nn.apply(eval_model, VAE(config))({"params": state.params, "batch_stats": state.batch_stats})


def vae_eval(config: VAEConfig, rng: KeyArray, state: TrainState, val_inp: jax.Array, latent_vec: jax.Array) -> jax.Array:
  def eval_model(vae_model: VAE):
    img_size = val_inp.shape[-3:]
    recon, mean, logvar = vae_model(rng, val_inp, False)
    comparison = jnp.concatenate([val_inp[:8].reshape(-1, *img_size),
...
def train_vae(rng: KeyArray, config: TrainConfig, train_ds, val_ds):
  ...
  init_inp = jnp.ones((1, config.model.image_size, config.model.image_size, 3), dtype=config.dtype)
  variables = model.init({"params": init_rng,
                       "dropout": dropout_rng},
                      reparam_rng, init_inp)
  bn = "batch_stats" in variables.keys()
  tx = optax.adam(learning_rate=create_learning_rate_fn(config, 10, config.lr, 46))
  state = TrainState.create(apply_fn=model.apply,
                            params=variables["params"],
                            tx=tx,
  ...
  for i_epoch in range(config.n_epochs):
    ...
    train_ds_iter = iter(train_ds.reshape(-1, config.batch_size, *train_ds.shape[-3:]))
    for minibatch in train_ds_iter:
      (state, rng), metrics = train_fn((state, rng), jnp.array(minibatch, dtype=config.dtype))
